...
from .cache import create_input_hash, load_cached_report, save_report
...
from .skill_matcher import calculate_job_match
...
def choose_cache_route(state: CareerPilotState) -> Literal["use_cache", "analyze_job"]:
    if state.get("cached_report"):
        print("[Router] Cached result found.")
        return "use_cache"
    print("[Router] No cache found. Running analysis.")
    return "analyze_job"
...
def analyze_job_node(state: CareerPilotState):
    print("[Node] Analysing job description...")
...
def match_resume_node(state: CareerPilotState):
    print("[Node] Calculating match score...")
...
def build_report_node(state: CareerPilotState):
    print("[Node] Building final report...")
