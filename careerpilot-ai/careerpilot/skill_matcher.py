...
def resume_has_skill(resume_text, skill):
    """Check a skill and its known aliases."""
...
def separate_skills(resume_text, job_skills):
    """Separate skills into matched and missing lists."""
    matched, missing = [], []
    for skill in job_skills:
        (matched if resume_has_skill(resume_text, skill) else missing).append(skill)
...
def calculate_job_match(resume_text, job_data):
    """Compare the résumé with analysed job requirements."""
